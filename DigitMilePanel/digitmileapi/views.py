...
from .serializers import (
    ...
    RunIngestionSerializer,
    UnityRunUploadPayloadSerializer,
)
...
class InsertRunDataView(APIView):
    """
    Endpoint for ingesting complete run data from Unity game.

    POST /panel/api/insertRunData/

    Expected payload from Unity (RunUploadPayload):
    {
        "classroomKey": "ABC123",
        "user": "Student Name",
        ...
        "run": {
            ...
            "correct_moves": 8,
            "wrong_moves": 2,
            "runStartedUnixMs": 1234567890000,
            "runEndedUnixMs": 1234567899000,
            "gameMap": { "mapTiles": [...] },
            "turns": [...]
        }
    }
    ...
    Features:
    - Generates run ID server-side (ignores Unity's runId)
    - Uses Unity-provided score, place, correct_moves, wrong_moves
    - Stores game map for replay/analysis
    - Atomic: All data is inserted in a single transaction
    - Bulk optimized: Uses bulk_create for turn events and triggers
    - Validates student exists by userID
    """

    throttle_classes = [AnonRateThrottle]

    def post(self, request, *args, **kwargs):
        serializer = UnityRunUploadPayloadSerializer(data=request.data)
        ...
        # Use Unity-provided values (already validated by serializer)
        ...
        try:
            with transaction.atomic():
                # Create the Run (ID generated by Django)
                ...
                # Prepare TurnEvent objects for bulk creation
                turn_events_to_create = []
                turn_event_triggers_map = {}  # turn_index -> list of triggers

                for event_data in turns_data:
                    # Convert Unix timestamp to timezone-aware datetime
                    timestamp_ms = event_data["timestampPlayedUnixMs"]
                    timestamp_played = datetime.fromtimestamp(
                        timestamp_ms / 1000.0, tz=dt_timezone.utc
                    )

                    # Extract position and tile data using correct Unity field names
                    pos_before = event_data["playerPositionBefore"]
                    pos_after = event_data["playerPositionAfter"]
                    tile_before = event_data["tileBefore"]
                    ...
                    turn_event = TurnEvent(
                        ...
                        tile_before_type=tile_before[
                            "tileIndex"
                        ],  # Using tileIndex as type
                        tile_after_index=pos_after[
                            "tileMapIndex"
                        ],  # Player's new position
                        place_before=pos_before["placeRelativeToBots"],
                        place_after=pos_after["placeRelativeToBots"],
                        bot_positions_before=event_data.get("botPositionsBefore", []),
                        bot_positions_after=event_data.get("botPositionsAfter", []),
                        card_decision_time_ms=event_data["cardDecisionTimeMs"],
                        offered_numbers=event_data.get("offeredNumbers", []),
                        chosen_number=event_data.get("chosenNumber"),
                        number_decision_time_ms=event_data.get("numberDecisionTimeMs"),
                    )
                    turn_events_to_create.append(turn_event)

                    # Store triggers for later bulk creation
                    triggers = event_data.get("specialTileTriggers", [])
                    if triggers:
                        turn_event_triggers_map[event_data["turnIndex"]] = triggers

                # Bulk create turn events
                created_turn_events = TurnEvent.objects.bulk_create(
                    turn_events_to_create
                )

                # Create index mapping for turn events
                turn_index_to_event = {
                    event.turn_index: event for event in created_turn_events
                }

                # Prepare SpecialTileTrigger objects for bulk creation
                triggers_to_create = []
                for turn_index, triggers in turn_event_triggers_map.items():
                    turn_event = turn_index_to_event[turn_index]
                    for trigger_data in triggers:
                        special_tile = trigger_data["specialTile"]
                        pos_on_special = trigger_data["positionOnSpecialTile"]
                        pos_after_effect = trigger_data["positionAfterEffect"]

                        trigger = SpecialTileTrigger(
                            turn=turn_event,
                            chain_index=trigger_data["chainIndex"],
                            special_tile_index=special_tile["tileMapIndex"],
                            special_tile_type=special_tile[
                                "tileIndex"
                            ],  # Using tileIndex as type
                            effect_delta_tiles=trigger_data["effectDeltaTiles"],
                            target_tile_index=pos_after_effect["tileMapIndex"],
                            target_tile_type=0,  # Not available in Unity data, default to 0
                            place_before=pos_on_special["placeRelativeToBots"],
                            place_after=pos_after_effect["placeRelativeToBots"],
                        )
                ...
                logger.info(
                    f"Successfully ingested run {run.id} for student {student_id}: "
                ...
                return Response(
                    {
                        "message": "Run data inserted successfully",
                        "run_id": str(run.id),
            ...
            logger.error(f"Integrity error ingesting run for student {student_id}: {e}")
            return Response(
            ...
            logger.exception(f"Error ingesting run for student {student_id}: {e}")
            return Response(
...
class RunIngestionView(APIView):
    ...
    Features:
    - Idempotent: Returns 200 if run with same UUID already exists
    - Atomic: All data is inserted in a single transaction
    - Bulk optimized: Uses bulk_create for turn events and triggers
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Handle race condition where run was created between check and insert
            if (
                ...
                return Response(
                    {
                        "message": "Run already ingested (race condition)",
                        "run_id": str(run_id),
                    },
                    status=status.HTTP_200_OK,
                )
            logger.error(f"Integrity error ingesting run {run_id}: {e}")
            return Response(
                {"error": "Database integrity error", "details": str(e)},
                status=status.HTTP_409_CONFLICT,
            )
            ...
            logger.exception(f"Error ingesting run {run_id}: {e}")
            return Response(
                {"error": "Internal server error while saving run data"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
